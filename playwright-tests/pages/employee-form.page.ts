...
export class EmployeeFormPage {
  ...
  async getLabelTextForTestId(testId: string): Promise<string | null> {
    await this.page.waitForSelector(`[data-testid="${testId}"]`, { state: 'visible' });
    return this.page.locator(`[data-testid="${testId}"]`).evaluate((input: Element) => {
  ...
  async getPhoneFieldLabel(): Promise<string | null> {
    return this.getLabelTextForTestId('phone-input');
  }

  async getCellPhoneFieldLabel(): Promise<string | null> {
    return this.getLabelTextForTestId('cellPhone-input');
  }

  async getAllFormLabels(): Promise<string[]> {
    await this.page.waitForSelector(this.employeeDrawer, { state: 'visible' });
  ...
  async countLabelsWithExactText(text: string): Promise<number> {
    await this.page.waitForSelector(this.employeeDrawer, { state: 'visible' });

...
test.describe('health — API Regression Suite', () => {
  ...
  test.describe('positive', () => {
    ...
    test('[API] health: GET /api/health returns 200 with ok status and mongodb info', async ({ page }) => {
  ...
  test.describe('negative', () => {
    ...
    test('[API] health: POST /api/health returns 404 or 405 Method Not Allowed', async ({ page }) => {
      await setupHealthMocks(page);
      await page.goto('/');
  ...
  test.describe('edge', () => {
    ...
    test('[API] health: GET /api/health with unexpected query params and headers still returns 200 ok', async ({ page }) => {
      await setupHealthMocks(page);
      await page.goto('/');
      ...
      const r2 = await page.evaluate(async () => {
        const res = await fetch('/api/health', {
          method: 'GET',
          headers: { 'Content-Type': 'application/json', 'X-Custom-Header': 'test-value', 'X-Request-Id': '999' },
        });
...
test.describe('health — API New Feature', () => {
  ...
  test.describe('positive', () => {
    test.skip('No new feature cases defined', () => {});
  });

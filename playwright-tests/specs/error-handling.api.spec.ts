...
test.describe('error-handling — API Gap Cases', () => {
  ...
  test.describe('negative', () => {
    ...
    test('GET /api/unknown returns 404 NOT_FOUND with JSON error body', async ({ page }) => {
      ...
      expect(JSON.stringify(r.body)).not.toMatch(/stack/i);
      expect(JSON.stringify(r.body)).not.toMatch(/node_modules/);
      expect(JSON.stringify(r.body)).not.toMatch(/\.ts:/);
      expect(JSON.stringify(r.body)).not.toMatch(/\.js:/);
    });
  });

  test.describe('edge', () => {
    // TC-7b8d2ab9-445d-50fa-4bfa-5a685b5dbfa0  SCOPE:regression
    test('deeply nested unknown API route and bare /api/ both return 404', async ({ page }) => {
      await setupErrorHandlingMocks(page);
      await page.goto('/');
      ...
      expect(JSON.stringify(r1.body)).not.toMatch(/stack/i);
